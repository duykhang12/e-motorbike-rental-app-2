...
Member::Member(string username, string password, string fullname, string email, string id_type,
               string id, string phone_number, string licen_number, string expiry_date) : username(username), password(password), fullname(fullname), email(email), id_type(id_type),
                                                                                          id(id), phone_number(phone_number), licen_number(licen_number), expiry_date(expiry_date) {}

Member::Member(string username, string password, string fullname, string email, string id_type,
               string id, string phone_number, string licen_number, string expiry_date, Motorbike *ownermotorbike) : username(username), password(password), fullname(fullname), email(email), id_type(id_type),
                                                                                                                     id(id), phone_number(phone_number), licen_number(licen_number), expiry_date(expiry_date), ownermotorbike(ownermotorbike) {}

Member::Member(string username, string password, string fullname, string email, string id_type,
               string id, string phone_number, string licen_number, string expiry_date, double cp, double rating) : username(username), password(password), fullname(fullname), email(email), id_type(id_type),
...
void Member::memberMenu(Admin &ad)
{
    ...
    do
    {
        ...
        case 2:
            ...
            break;
        case 3:
            registerMotorbike(ad);
            break;
        case 4:
            searchMotorbikes(ad);
            break;
        case 5:
            makeRentalRequest(ad);
            break;
        case 6:
            viewIncomingRequests(ad);
            break;
        case 7:
            completeRental(ad);
            break;
...
void Member::viewMotorbike()
{
    ...
    while (getline(file, line))
    {
        ...
        if (ownerName == this->username)
        {
            ...
            cout << "Min Renter Rating: " << minRating << endl;
            cout << "------------------------------------\n";
        }
    }
    if (!found)
...
void Member::registerMotorbike(Admin &ad)
{
    ...
    Motorbike *newBike = new Motorbike(brand, model, color, size, year, plate, location, dailyRate, minRating, this->username, availableFrom, availableTo);
...
void Member::makeRentalRequest(Admin &ad)
{
    ...
    RentalRequest *newReq = new RentalRequest(ad.allRequests.size() + 1, this, owner, bike, day); // days will be calculated later
